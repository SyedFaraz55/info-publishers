...
const Distributors = () => {
  ...
  const getDistById = async () => {
    console.log(current)
    const result = await axios.get(`http://localhost:8000/api/admin/get-distById/${current._id}`);
                    ...
                    <Tr style={{ background: "#fff" }}>
                      ...
                      <Td>
                        <Select onChange={(e) => {
                          ...
                          axios.post("http://localhost:8000/api/admin/update-dist", { id: item._id, status: e.target.value })
                            .then(res => {
                              if (res.data.ok) {
