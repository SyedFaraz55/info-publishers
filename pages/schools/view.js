...
const Series = () => {
  ...
  const getStudents = async () => {
    const result = await axios.get(
      `http://13.235.100.69:8000/api/admin/get-student/${router.query.q}`
    );
  ...
  const handleDelete = async (item) => {
    ...
    if (ret) {
      const result = await axios.post(
        "http://13.235.100.69:8000/api/admin/delete-student",
        { id: item._id }
      );
      if (result.data.ok) {
                    ...
                    <Tr key={item._id} style={{ background: "#fff" }}>
                      ...
                      <Td>
                        <Select onChange={(e) => {
                          ...
                          axios.post("http://13.235.100.69:8000/api/admin/update-student", { id: item._id, status: e.target.value })
                            .then(res => {
                              if (res.data.ok) {
