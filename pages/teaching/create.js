...
const Create = () => {
    ...
    const [state,setState] = useState({name:"",link:"",date:""})
    ...
    const handleSubmit =async ()=> {
        try {
            const result = await axios.post(
              ...
              { ...state, id:selectedLesson ,series,class:selectedClass,subject:selectedSubject}
            );
            if (result.data.ok) {
              setLoading(false);
            ...
            <Layout>
                <Box p={4} width={500}>
                    ...
                    <FormControl  mt={2} isRequired>
                       ...
                       <Input bg="#fff" type={"date"} placeholder="date" name="date" onChange={e => setState(prevState => ({...prevState,date:e.target.value}))} />
                    </FormControl>
                    {loading ? (
                        <Spinner mt={2} />
                    ) : (
